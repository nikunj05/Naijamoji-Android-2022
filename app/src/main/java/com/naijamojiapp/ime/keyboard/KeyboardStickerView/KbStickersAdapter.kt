...
class KbStickersAdapter(private val mcontext: Context,
                        val mEmojiList: ArrayList<AllStickerList>?,
                        val mDaynamicEmojilist: ArrayList<StickerListByTagOrCategory.Result>?,
                        val sendStickersInterface: SendStickersInterface,
                        val mSendDaynamicStickersInterface: SendDaynamicStickersInterface,
                        val size: Int) : RecyclerView.Adapter<KbStickersAdapter.MyViewHolder>() {
    var image = null
    var mHolder: MyViewHolder? = null

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val v = LayoutInflater.from(mcontext).inflate(R.layout.item_sticker_iv_new, null, true)
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        try {
            if (size != null && size > 0) {
                mHolder!!.ivSticker.layoutParams.width = dpToPx(size)
                mHolder!!.ivSticker.layoutParams.height = dpToPx(size)
            } else {
                mHolder!!.ivSticker.layoutParams.width = dpToPx(70)
                mHolder!!.ivSticker.layoutParams.height = dpToPx(70)
            }
        } catch (e: Exception) {
        }
    ...
    fun loadImage(mUrl: String) {
        Picasso.get()
                .load(mUrl)
                .into(mHolder!!.ivSticker, object : Callback {
    ...
    fun dpToPx(dp: Int): Int {
        val density = mcontext.getResources()
                .getDisplayMetrics()
                .density
